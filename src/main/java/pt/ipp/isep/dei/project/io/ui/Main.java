...
@ComponentScan("pt.ipp.isep.dei.project")
...
public class Main {
    ...
    public CommandLineRunner mainRun() {
        /*sensorReadingsRepository = Repositories.getInstance().getSensorReadingsRepository();
        sensorRepository = Repositories.getInstance().getSensorRepository();
        geoAreaRepository = Repositories.getInstance().getGeoAreaRepository();
        sensorListRepository = Repositories.getInstance().getSensorListRepository();*/

        return (args) -> {

            String configFile = "Configuration.properties";

            LocalDateTime ewhDate1338 = LocalDateTime.of(2018, 12, 31, 21, 15, 00);
            Reading reading = new Reading(23, ewhDate1338);

            LocalDateTime startingDate = LocalDateTime.of(1991, 11, 2, 21, 10, 25);
            SensorType temperature = new SensorType("Temperature");
            Location location = new Location(123, 345, 50);
            Sensor temperatureSensor = new Sensor("R003", "A123", startingDate, temperature, location, "l/m2");
